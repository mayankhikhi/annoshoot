...
@onready var shotgun_system = $ShotgunSystem
...
# ### TACKLE MECHANICS
const TACKLE_COOLDOWN = 1.2  # Seconds between tackles
const TACKLE_FORCE = 300.0   # Knockback force on enemies
const TACKLE_RADIUS = 90.0  # Area of effect radius
const TACKLE_DAMAGE = 15.0   # Damage dealt
var can_tackle = true
...
func handle_tackle():
	if Input.is_action_just_pressed("tackle") and can_tackle and not is_tackling:
		execute_tackle()

func execute_tackle() -> void:
	"""Execute tackle attack - knockback enemies in radius"""
	# Prevent spam - don't allow tackle if already tackling
	if is_tackling:
		return
	
	# Validate the node is still in the scene tree
	...
	var nearby_nodes = tree.get_nodes_in_group("enemy")
	nearby_nodes += tree.get_nodes_in_group("human")
	
	# Apply knockback to enemies within range
	for body in nearby_nodes:
		...
		# Apply knockback - try different methods
		if body.get_script() != null and "knockback_vector" in body:
			body.knockback_vector = direction_to_enemy * TACKLE_FORCE
		elif body.has_method("push"):
			body.push(direction_to_enemy * TACKLE_FORCE)
		elif body is RigidBody2D:
			body.velocity = direction_to_enemy * TACKLE_FORCE
		elif body is CharacterBody2D:
			if "knockback_vector" in body:
				body.knockback_vector = direction_to_enemy * TACKLE_FORCE
			else:
				body.velocity = direction_to_enemy * TACKLE_FORCE
	...
	# Reset tackling state
	is_tackling = false
	
	# Cooldown with safety check
	var cooldown_tree = get_tree()
	if cooldown_tree != null:
		await cooldown_tree.create_timer(TACKLE_COOLDOWN).timeout
...
func fire_shotgun() -> void:
	...
	shotgun_system.fire_shotgun(
		global_position + Vector2(40, 0),
		fire_direction,
		self,
